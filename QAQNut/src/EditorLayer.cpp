...
namespace QAQ
{
	...
	void EditorLayer::OpenScene()
	{
		if (m_SceneState != SceneState::Edit)
			OnSceneStop();
		std::string filepath = FileDialogs::OpenFile("QAQ Scene (*.qaq)\0*.qaq\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path &path)
	{
		...
		{
			QAQ_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		QAQ_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
