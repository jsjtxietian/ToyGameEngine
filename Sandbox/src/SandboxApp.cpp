...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader.reset(new QAQ::Shader(vertexSrc, fragmentSrc));
	...
	void OnUpdate(QAQ::TimeStep ts) override
	{
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				QAQ::Renderer::Submit(m_BlueShader, m_SquareVA, transform);
			}
		}

		QAQ::Renderer::Submit(m_Shader, m_VertexArray);
...
private:
	...
	std::shared_ptr<QAQ::Shader> m_BlueShader;
