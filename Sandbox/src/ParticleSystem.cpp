...
void ParticleSystem::OnRender(QAQ::OrthographicCamera& camera)
{
	QAQ::Renderer2D::BeginScene(camera);
	for (auto& particle : m_ParticlePool)
	{
		if (!particle.Active)
			continue;
		...
		QAQ::Renderer2D::DrawRotatedQuad(position, { size,size }, particle.Rotation, color);
	}
